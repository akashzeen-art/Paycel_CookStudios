...
export default function Classes() {
  ...
  const classes = [
    ...
    },
    {
      ...
      style: "Acro",
      image: "🤸",
      description: "Partner acrobatic Cook",
      rating: 4.9,
    },
    {
      id: 18,
  ...
  useEffect(() => {
    const filtered = classes.filter((CookClass) => {
      const matchesSearch =
        CookClass.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        CookClass.style.toLowerCase().includes(searchTerm.toLowerCase()) ||
        CookClass.description.toLowerCase().includes(searchTerm.toLowerCase());

      let matchesFilter = false;
      if (activeFilter === "All") {
        matchesFilter = true;
      } else if (activeFilter === "Beginner") {
        matchesFilter = CookClass.difficulty === "Beginner" || CookClass.difficulty === "Easy";
      } else if (activeFilter === "Intermediate") {
        matchesFilter = CookClass.difficulty === "Medium";
      } else if (activeFilter === "Advanced") {
        matchesFilter = CookClass.difficulty === "Hard";
      } else if (activeFilter === "Meditation") {
        matchesFilter = CookClass.style === "Mindfulness" || CookClass.style === "Nidra";
      } else if (activeFilter === "Flow") {
        matchesFilter = CookClass.style === "Vinyasa" || CookClass.style === "Hatha";
      }
          ...
          <div className="max-w-7xl mx-auto">
            ...
            {filteredClasses.length > 0 ? (
              <Carousel slidesToShow={5}>
                {filteredClasses.map((CookClass, index) => (
                  <div key={CookClass.id} className="group cursor-pointer transform transition-all duration-300 hover:scale-105" onClick={() => openVideo(videoUrls[index % videoUrls.length], CookClass.name)}>
                    <div className="relative aspect-video rounded-lg overflow-hidden shadow-md group-hover:shadow-xl transition-all duration-300">
                      <img src={`/thumbnails/${['F1', 'F2', 'F3', 'F4', 'F5', 'F6', 'F7', 'F8', 'P1', 'P2', 'P3', 'P4', 'P5', 'P6', 'P7', 'P8', 'P9', 'P10', 'S1', 'S2'][index % 20]}.png`} alt={CookClass.name} className="w-full h-full object-cover" />
                    ...
                    <div className="mt-2">
                      <h3 className="font-bold text-sm text-white">{CookClass.name}</h3>
                      <p className="text-xs text-white/80">{CookClass.time}</p>
                    </div>
                  </div>
                ))}
              </Carousel>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-10">
              {[
                ...
                { id: 3, title: "Daily Cook", time: "00:42 mins" },
                ...
                <div key={video.id} className="group cursor-pointer animate-fade-in" style={{ animationDelay: `${index * 50}ms` }} onClick={() => openVideo(videoUrls[14 + index], video.title)}>
                  ...
                  <div className="mt-2">
                    <p className="text-xs text-white/80">{video.time}, Cook</p>
